...
class Controller : Initializable {
    ...
    @FXML fun convertList() {
        ...
        if (playlistLink.text.isEmpty()) {
            errPaner("Nem lehet üres")
        } else if (!playlistLink.text.contains("https://www.youtube.com/playlist?list=")) {
            if (playlistLink.text.startsWith("youtube.com/playlist?list=") || playlistLink.text.startsWith("www.youtube.com/playlist?list=")) {
            ...
            try {
                ...
                when (type) {
                    "mp4" -> {
                        ...
                        fxScope.launch { outputLabel.text = "A videó letöltése elkezdődött..." }
                    }
                    "mp3" -> {
                        println("mp3")
                        ...
                        fxScope.launch { outputLabel.text = "Az audió letöltése elkezdődött..." }
                    }
                    "legjobb videó" -> {
                        println("vidBest")
                        builder.command(
                            "cmd.exe",
                            "/c",
                            pathToYTdlEXE + " -f best -o " + path.text + "\\%(title)s.%(ext)s " + link.text
                        )
                        fxScope.launch { outputLabel.text = "A videó letöltése elkezdődött..." }
                    }
                    "legjobb hang" -> {
                        ...
                        builder.command(
                            "cmd.exe",
                            "/c",
                            pathToYTdlEXE + " -x -f best -o " + path.text + "\\%(title)s.%(ext)s " + link.text
                        )
                        fxScope.launch { outputLabel.text = "A hang letöltése elkezdődött..." }
                    }
                }
                ...
                try {
                    ...
                    while (true) {
                        ...
                        println(line)
                        fxScope.launch {
                            //outputLabel.text += line.split("%")[0].split("]")[1]
                            outLabel.text += "Henlo"
                            outLabel.text = outLabel.text + line
                        }
                    }
